...
re_verification:
  ...
  gaps_remaining:
    - "G-04-4: SIGNUP_LOCKED UI copy on prod after second register — code fix shipped (sessionStorage persistence), prod UAT re-run deferred"
  regressions: []
...
human_verification:
  ...
  - test: "Prod UAT #6 SIGNUP_LOCKED UI — VOICE copy nach zweiter Registrierung"
    ...
    why_human: "Code fix shipped (sessionStorage 'pb.signup_locked'), unit test grün, aber prod repro braucht echten Browser + zweite Register-Attempt"
...
| AUTH-03 | 04-05 | Signup-Lock nach erstem Account | ✓ SATISFIED | API 409 confirmed via curl; sessionStorage UI fix shipped; prod UAT #6 ausstehend |
...
3. **Prod UAT #6 SIGNUP_LOCKED UI** — VOICE copy nach zweiter Registrierung (G-04-4 code fix shipped, prod repro ausstehend)
...
**Geschlossen (7/8):** G-04-1, G-04-2, G-04-3, G-04-5, G-05-1, G-05-2, G-05-3 — alle Infra-Blocker via prod curls + code inspection verifiziert. CORS preflight grün, JWKS 200, NEXT_PUBLIC bake bewiesen (pbox.puzzlesstool.online in chunk), apollo-onboard.png 200, signup 409.

**Verbleibend (1/8 partial):** G-04-4 — code fix shipped (sessionStorage `pb.signup_locked` persistence über Remount), unit test grün, aber prod UAT #6 repro ausstehend. Plan 04-05 Task 5 erlaubte explizit `verify-after-connectivity` falls repro nach Connectivity-Fix nicht möglich im executor — accepted per plan.
